...
class AccountPayment(models.Model):
    ...
    def _compute_payment_method_description(self):
        for rec in self:
            rec.payment_method_description = rec.payment_method_line_id.name or rec.payment_method_line_id.payment_method_id.display_name
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        if self.force_amount_company_currency:
            # In v17+ move lines use 'balance' field (positive=debit, negative=credit)
            # res[0] = liquidity line, res[1] = counterpart line
            liquidity_balance = res[0].get('balance', 0.0)
            if liquidity_balance >= 0:
                # debit side: balance is positive
                difference = self.force_amount_company_currency - liquidity_balance
                res[0]['balance'] = self.force_amount_company_currency
                res[1]['balance'] = res[1].get('balance', 0.0) - difference
            else:
                # credit side: balance is negative
                difference = self.force_amount_company_currency - abs(liquidity_balance)
                res[0]['balance'] = -self.force_amount_company_currency
                res[1]['balance'] = res[1].get('balance', 0.0) + difference
        return res
