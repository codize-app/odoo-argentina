    ...
    def create(self, vals):
        ...
        if len(partner)>0:
            ...
            if padron.type_alicuot == 'R':
                for alicuota in partner.alicuot_ret_sircar_neuquen_ids:
                    if alicuota.padron_activo == True:
                        alicuota.padron_activo = False
                partner.sudo().update({'alicuot_ret_sircar_neuquen_ids' : [(0, 0, {
                ...
                for alicuota in partner.alicuot_per_sircar_neuquen_ids:
                    if alicuota.padron_activo == True:
                        alicuota.padron_activo = False
                partner.sudo().update({'alicuot_per_sircar_neuquen_ids' : [(0, 0, {
                    'partner_id': partner.id, 
                    'publication_date': padron.publication_date,
                    'effective_date_from': padron.effective_date_from,
                    'effective_date_to': padron.effective_date_to,
            ...
            if len(partner)>0:
                ...
                if self.type_alicuot == 'R':
                    for alicuota in partner.alicuot_ret_sircar_neuquen_ids:
                        if alicuota.padron_activo == True:
                            alicuota.padron_activo = False
                    partner.alicuot_ret_sircar_neuquen_ids = [(0, 0, {
                        'partner_id': partner.id, 
                    ...
                    for alicuota in partner.alicuot_per_sircar_neuquen_ids:
                        if alicuota.padron_activo == True:
                            alicuota.padron_activo = False
                    partner.alicuot_per_sircar_neuquen_ids = [(0, 0, {
                        'partner_id': partner.id, 
                        'publication_date': self.publication_date,
                        'effective_date_from': self.effective_date_from,
                        'effective_date_to': self.effective_date_to,
