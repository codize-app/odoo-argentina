    ...
    def generate_xlsx_report(self, workbook, data, vat_ledger):
        if vat_ledger.invoice_ids:
            ...
            for i,obj in enumerate(vat_ledger.invoice_ids):
                ...
                percaba = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB CABA':
                        percaba = percaba + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    percaba = percaba * -1
                ...
                perarba = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB ARBA':
                        perarba = perarba + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    perarba = perarba * -1
                ...
                perstafe = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Santa Fe':
                        perstafe = perstafe + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    perstafe = perstafe * -1
                ...
                percordoba = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Córdoba':
                        percordoba = percordoba + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                ...
                permend = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Mendoza':
                        permend = permend + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    permend = permend * -1
                ...
                perpampa = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB La Pampa':
                        perpampa = perpampa + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    perpampa = perpampa * -1
                ...
                persalta = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Salta':
                        persalta = persalta + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    persalta = persalta * -1
                ...
                performosa = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Formosa':
                        performosa = performosa + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                ...
                percorr = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Corrientes':
                        percorr = percorr + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    percorr = percorr * -1
                ...
                persanl = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB San Luis':
                        persanl = persanl + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    persanl = persanl * -1
                ...
                perrine = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Rio Negro':
                        perrine = perrine + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    perrine = perrine * -1
                ...
                perstacr = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Santa Cruz':
                        perstacr = perstacr + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    perstacr = perstacr * -1
                ...
                pertie = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Perc IIBB Tierra del Fuego':
                        pertie = pertie + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    pertie = pertie * -1
                ...
                impinter = 0
                for tax in obj.amount_by_group:
                    if tax[0] == 'Impuestos Internos':
                        impinter = impinter + tax[1]
                if obj.l10n_latam_document_type_id.internal_type == 'credit_note':
                    impinter = impinter * -1
