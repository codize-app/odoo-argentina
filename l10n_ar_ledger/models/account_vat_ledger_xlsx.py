...
class AccountVatLedgerXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, vat_ledger):
        if vat_ledger.invoice_ids:
            ...
            titles = ['Fecha','Razón Social','CUIT','Responsabilidad AFIP','Tipo de Comprobante','Nro Comprobante','Neto gravado','Neto no gravado','Neto exento','IVA 27%','IVA 21%','IVA 10.5%','Percepción de IVA','Perc IIBB','Percepciones Municipales','Otras Percepciones','Impuestos Internos','Otros','Total gravado','Total']
            ...
            for i,obj in enumerate(vat_ledger.invoice_ids):
                ...
                sheet.write(row + index, 6, netoG * credit, money_format) # Neto gravado

                netoN = amounts['vat_untaxed_base_amount']
                sheet.write(row + index, 7, netoN * credit, money_format) # Neto no gravado
                ...
                sheet.write(row + index, 12, perc_iva * credit, money_format) # Perpceción IVA

                perc_iibb = amounts['iibb_perc_amount']
                sheet.write(row + index, 13, perc_iibb * credit, money_format) # Perpceción IIBB

                perc_mun = amounts['mun_perc_amount']
                sheet.write(row + index, 14, perc_mun * credit, money_format) # Percepciones Municipales

                perc_other = amounts['other_perc_amount']
                sheet.write(row + index, 15, perc_other * credit, money_format) # Otras Percepciones

                internal_taxes = amounts['intern_tax_amount']
                sheet.write(row + index, 16, internal_taxes * credit, money_format) # Impuestos Internos
                ...
                sheet.write(row + index, 18, total_net * credit, money_format) # Total Gravado
