...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            sale_orders._set_currency_invoice(self.new_currency_id, self.tipo_cambio)
    ...
    @api.depends("new_currency_id")
    def _get_tasa_currency(self):
        sale_orders = self.env['sale.order'].browse(self._context.get('active_ids', []))
        for order in sale_orders:
        # for rec in self:
            if (order.currency_id.name != self.new_currency_id.name):
                if order.currency_id.name == 'ARS':
                    rate = self.new_currency_id.rate
                else:
    ...
    tipo_cambio = fields.Float(string='Tipo de cambio', compute=_get_tasa_currency, store=True, readonly=False, digits=(12, 6))
