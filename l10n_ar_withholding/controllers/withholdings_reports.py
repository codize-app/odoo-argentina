            ...
            for group in invoice.amount_by_group:
                if 'Perc' in group[0]:
                    if invoice.move_type == 'out_invoice':
                        _base = group[2] if invoice.currency_id.name == 'ARS' else float(group[2]) * invoice.l10n_ar_currency_rate #Multimoneda
                        _moto_per = group[1] if invoice.currency_id.name == 'ARS' else float(group[1]) * invoice.l10n_ar_currency_rate #Multimoneda
                    else:
                        _base = (float(group[2]) * -1) if invoice.currency_id.name == 'ARS' else ((float(group[2]) * invoice.l10n_ar_currency_rate) * -1) #Multimoneda
                        _moto_per = (float(group[1]) * -1) if invoice.currency_id.name == 'ARS' else ((float(group[1]) * invoice.l10n_ar_currency_rate) * -1) #Multimoneda
                    ...
                    if not group[0] in _precepcion_categories:
                        _vals={'impuesto' : group[0],
                                'Base': _base,
                                'MontoPer': _moto_per}
                        _precepcion.append(_vals)
                        _precepcion_categories.append(group[0])
                    # En el caso de que ya se haya cargado la categoria se suman los totales del producto de dicha categoria
                    else:
                        for per in _precepcion:
                            if per['impuesto'] == group[0]:
