...
class ResCompany(models.Model):
        ...
        * other or no parameter -->  production
        ...
        if parameter_env_type == 'production':
            ...
            if not server_mode or server_mode == 'production':
                ...
                environment_type = 'homologation'
        _logger.info('Factura electrónica Arg. en modo %s' % environment_type)
        return environment_type

    def get_key_and_certificate(self, environment_type):
        """
        Función que busca para el environment_type definido,
        ...
        * en registros de esta misma clase
        ...
        if certificate:
            ...
            _logger.info('Usando Certificados de la DB')
        else:
            msg = _('No hay certificado confirmado para %s de la compañía %s') % (
            ...
            if pkey_path and cert_path:
                try:
                    ...
                    _logger.info('Usando Certificados del odoo.conf')
    ...
    def get_connection(self, afip_ws):
        self.ensure_one()
        _logger.info('Obteniendo conexión para la compañía %s y WS %s' % (self.name, afip_ws))
    ...
    @api.model
    def authenticate(self, service, certificate, private_key, force=False, cache="", wsdl="", proxy=""):
        """
        ...
        try:
            ...
            raise UserError('No pudimos conectarnos. Esto es lo que recibimos: %s' % err_msg)
        return {
            'uniqueid': uniqueId,
