...
print "Observaciones:", wscdc.Obs
    ...
    def do_pyafipws_request_cae(self):
        ...
        for inv in self:
            ...
            # Autentificación con ARCA
            ws = inv.company_id.get_connection(afip_ws).connect()
    
            if afip_ws == 'wsfex':
                if not country:
                    raise UserError(_(
                        'Para el WS "%s" el país es requerido en el Cliente' % (
                            afip_ws)))
                elif not country.code:
                    raise UserError(
                        'Para el WS "%s" el código del país es obligatorio\n'
                        'Pais: %s' % (afip_ws, country.name))
                elif not country.l10n_ar_afip_code:
                    raise UserError(
                        'Para el WS "%s" el código del pais en ARCA es obligatorio\n'	
                        'País: %s' % (afip_ws, country.name))
    
            ws_next_invoice_number = int(inv.l10n_latam_document_type_id.get_pyafipws_last_invoice(inv)['result']) + 1
            ...
            # Totales de Factura:
            amount_total = amounts["vat_taxable_amount"] + amounts["vat_amount"] + amounts["vat_exempt_base_amount"] + amounts["other_taxes_amount"] + amounts['iibb_perc_amount'] + amounts["vat_perc_amount"] + amounts["profits_perc_amount"] + amounts["other_perc_amount"] + amounts['mun_perc_amount'] + amounts["vat_untaxed_base_amount"]
            # Total Factura:
            imp_total = str("%.2f" % amount_total)
            # Total IVA No Gravado:
            ...
            if inv.l10n_latam_document_type_id.l10n_ar_letter == 'C':
                imp_neto = str("%.2f" % inv.amount_untaxed) # Total Neto para Facturas C, sin IVA
            # Total de Tributos:
            ...
            if not moneda_id:
                raise ValidationError('No está definido el codigo ARCA en la moneda')
    
            # CrearFactura con WSFE: Factura Electrónica Regular
            if afip_ws == 'wsfe':
            ...
            # CrearFactura con WSFEX: Factura Electrónica de Exportación
            elif afip_ws == 'wsfex':
                # # foreign trade data: export permit, country code, etc.:
                if inv.invoice_incoterm_id:
                    incoterms = inv.invoice_incoterm_id.code
                    incoterms_ds = inv.invoice_incoterm_id.name
                    incoterms_ds = incoterms_ds and incoterms_ds[:20]   # Incoterms max 20 chars
                else:
                ...
                idioma_cbte = 1     # Idioma de la Factura: Español
                ...
                if nro_doc:
                    ...
                    if not cuit_pais_cliente:
                        raise UserError('No hay VAT definido para el cliente ni CUIT para el país del cliente')
                ...
                ws.CrearFactura(
                    ...
                    idioma_cbte, incoterms_ds, fecha_venc_pago
                )
    
            # CrearFactura con WSBFE: Bono Fiscal
            elif afip_ws == 'wsbfe':
                ...
                ws.CrearFactura(
                    ...
                    cbte_nro, fecha_cbte, imp_total, imp_neto, imp_iva, imp_tot_conc,
                    impto_liq_rni, imp_op_ex, imp_perc, imp_iibb, imp_perc_mun,
                    imp_internos, moneda_id, round(moneda_ctz,2), fecha_venc_pago
                )
    
            if afip_ws in ['wsfe', 'wsbfe']:
                if mipyme_fce:
                    ws.AgregarOpcional(opcional_id=2101, valor=inv.partner_bank_id.cbu) # CBU para FCE
                    ws.AgregarOpcional(opcional_id=27, valor=inv.afip_mypyme_sca_adc)   # SCA/ADC para FCE
                elif int(doc_afip_code) in [202, 203, 207, 208, 212, 213]:
                    valor = inv.afip_fce_es_anulacion and 'S' or 'N'
                    ws.AgregarOpcional(opcional_id=22, valor=valor) # Opcional 22 para anulación de FCE
    
            if afip_ws not in ['wsfex', 'wsbfe']:
                for vat_tax in self._build_afip_wsfe_vat_taxes():
                    ws.AgregarIva(**vat_tax)    # Agregar IVA en Facturas Regulares
            ...
            if CbteAsoc:
                ...
                # Factura E sin Fechas
                if afip_ws == 'wsfex':
                    ws.AgregarCmpAsoc(
                        CbteAsoc.l10n_latam_document_type_id.code,
                        CbteAsoc.journal_id.l10n_ar_afip_pos_number,
                        doc_number, self.company_id.vat
                    )
                else:
                    ws.AgregarCmpAsoc(
                        CbteAsoc.l10n_latam_document_type_id.code,
                        CbteAsoc.journal_id.l10n_ar_afip_pos_number,
                        doc_number, self.company_id.vat, invoice_date
                    )
            ...
            if afip_ws != 'wsfe':
                for line in inv.invoice_line_ids:
                    ...
                    if not line.product_uom_id:
                        ...
                        raise UserError('No existe el código ARCA para el UOM %s' % (line.product_uom_id.name))
                    else:
                    ...
                    if afip_ws in ['wsmtxca', 'wsbfe']:
                        ...
                        if afip_ws == 'wsmtxca':
                            raise UserError('WS wsmtxca no está implementado')
                        elif afip_ws == 'wsbfe':
                            sec = ""
                            ws.AgregarItem(codigo, sec, ds, qty, umed, precio, bonif, iva_id, importe + imp_iva)
                    elif afip_ws == 'wsfex':
            ...
            try:
                ...
                msg = 'Fallo en SOAP %s: %s' % (fault.faultcode, fault.faultstring)
            except Exception as e:
                msg = e
            except Exception:
                if ws.Excepcion:
            ...
            if msg:
                _logger.info('ARCA Validation Error. %s' % msg)+' XML Request: %s XML Response: %s' % (ws.XmlRequest, ws.XmlResponse)
                raise UserError('ARCA Error de Validación. %s' % msg)
