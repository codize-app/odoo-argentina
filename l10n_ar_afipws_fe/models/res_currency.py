...
class ResCurrency(models.Model):
    ...
    def get_pyafipws_currencies(self, afip_ws='wsfex', company=False):
        ...
        if afip_ws == 'wsfex':
            ...
            raise UserError('ARCA WS %s no implementado' % (afip_ws))
        msg = (_("Monedas Autorizadas en ARCA%s\n. \nObservations: %s") % (
    ...
    def get_pyafipws_currency_rate(self, afip_ws='wsfe', company=False):
        ...
        if not self.l10n_ar_afip_code:
            raise UserError('No hay código de moneda de ARCA %s' % self.name)
